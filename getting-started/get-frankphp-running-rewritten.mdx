...
FrankPHP now uses a standard [`.env` file for environment-specific credentials](/architecture/environment-configuration). This keeps secrets out of your main config structure and makes FrankPHP feel more familiar if you've used other PHP frameworks before.
...
If your download includes an `.env.example` file, copy that first and rename it to `.env`, then fill in your real values. The example file documents every key the application supports — including mail credentials needed for password reset — so it's the safer starting point.
...
1. `index.php` loaded `bootstrap.php`, which immediately called `Env::load()` to read `.env` into `$_ENV` before anything else ran
2. The database connection was established using credentials from `$_ENV`, then the router dispatched the request
3. The router matched `/tenant/1/hello` and identified your controller
